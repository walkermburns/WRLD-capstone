...
int main(int argc, char* argv[])
{
    ...
    for(;;)
    {
        ...
        {
            printf("Required number of frames reached. Running calibration now...\n");
        ...
        {
            printf("No more images to process. Running calibration now...\n");
...
bool runCalibrationAndSave(Settings& s, Size imageSize, Mat& cameraMatrix, Mat& distCoeffs,
                           vector<vector<Point2f> > imagePoints, float grid_width, bool release_object)
{
    printf("Running calibration on %d valid images...\n", (int)imagePoints.size());
    ...
    if (ok)
        cout << "Saving camera parameters to file \"" << s.outputFileName << "\"..." << endl;
