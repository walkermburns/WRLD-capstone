...
USE_QUAT_CONJUGATE = False  # per your final working state

GAIN = 1.0
MAX_TILT_RAD = math.radians(35.0)

# Smoothing (EMA) on correction angles
SMOOTH_ALPHA = 0.8  # smaller => smoother, more lag
...
glvideomixer name=mix background=black
  ...
  sink_1::xpos={RES_W} sink_1::ypos=0 !
video/x-raw(memory:GLMemory),format=RGBA,width={2*RES_W},height={RES_H} !
gldownload !
videoconvert !
textoverlay name=txt halignment=left valignment=top shaded-background=true font-desc="Sans 14" !
videoconvert !
glupload !
...
def main():
    ...
    txt = pipe.get_by_name("txt")
    if glshader is None:
        raise RuntimeError("Could not find glshader named 'stab'")
    if txt is None:
        raise RuntimeError("Could not find textoverlay named 'txt'")
    ...
    def probe_cb(pad, info):
        ...
        nonlocal latest_Hinv, have_update, latest_ypr_deg
        ...
        # Absolute angles for overlay
        ypr_cur = R_cur_flu.as_euler("zyx", degrees=True)
        with lock:
            latest_ypr_deg = (float(ypr_cur[0]), float(ypr_cur[1]), float(ypr_cur[2]))

        # Relative rotation (your working convention)
        R_rel = R_cur_flu * R_ref_flu.inv()

        # Roll/pitch-only correction
        ...
        if now - last_print_wall >= PRINT_EVERY_S:
            last_print_wall = now
            if bad_count > 0:
                print(f"[info] bad frames rejected so far: {bad_count}", flush=True)
    ...
    def apply_updates():
        ...
        txt.set_property(
            "text",
            f"Original | Stabilized\n"
            f"Yaw: {y: .2f} deg\nPitch: {p: .2f} deg\nRoll: {r_: .2f} deg"
        )
        return True

    GLib.timeout_add(33, apply_updates)
